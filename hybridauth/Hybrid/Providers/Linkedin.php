...
class Hybrid_Providers_LinkedIn extends Hybrid_Provider_Model
{
    ...
    function getUserProfile() {
        try {
			...
			$response = $this->api->profile('~:(id,first-name,last-name,public-profile-url,picture-url,email-address,date-of-birth,phone-numbers,summary,num-connections)');
        ...
        if (isset($response['success']) && $response['success'] === TRUE) {
            ...
            $this->user->profile->positions = (array)$data->{'positions'};
    ...
    function setUserStatus($status) {
        ...
        if (is_array($status)) {
            if (isset($status['title']) && !empty($status['title'])) {
                $parameters["title"] = $status['title'];
            } // post title
            if (isset($status['message']) && !empty($status['message'])) {
                $parameters["comment"] = $status['message'];
            } // post comment
            if (isset($status['link']) && !empty($status['link'])) {
                $parameters["submitted-url"] = $status['link'];
            } // post url
            if (isset($status['picture']) && !empty($status['picture'])) {
                $parameters["submitted-image-url"] = $status['picture'];
            } // post picture url
            if (isset($status['private']) && !empty($status['picture'])) {
